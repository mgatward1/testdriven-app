...
then
  ...
  if [[ "$TRAVIS_BRANCH" == "staging" ]]; then
    ...
    export REACT_APP_USERS_SERVICE_URL="testdriven-staging-alb-1028431689.eu-west-2.elb.amazonaws.com"
  ...
  then
    # users
    docker build $USERS_REPO -t $USERS:$COMMIT -f Dockerfile-$DOCKER_ENV  # new123
    ...
    # swagger
    docker build $SWAGGER_REPO -t $SWAGGER:$COMMIT -f Dockerfile-$DOCKER_ENV  # new123
